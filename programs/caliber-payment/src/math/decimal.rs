...
impl Decimal {
    ...
    pub fn to_scaled_val(&self) -> Result<u128, PaymentError> {
        Ok(u128::try_from(self.0).map_err(|_| PaymentError::MathOverflow)?)
    }
    ...
    pub fn try_round_u64(&self) -> Result<u64, PaymentError> {
        let rounded_val = Self::half_wad()
            .checked_add(self.0)
            .ok_or(PaymentError::MathOverflow)?
            .checked_div(Self::wad())
            .ok_or(PaymentError::MathOverflow)?;
        Ok(u64::try_from(rounded_val).map_err(|_| PaymentError::MathOverflow)?)
    }

    /// Ceiling scaled decimal to u64
    pub fn try_ceil_u64(&self) -> Result<u64, PaymentError> {
        let ceil_val = Self::wad()
            ...
            .checked_div(Self::wad())
            .ok_or(PaymentError::MathOverflow)?;
        Ok(u64::try_from(ceil_val).map_err(|_| PaymentError::MathOverflow)?)
    }

    /// Floor scaled decimal to u64
    pub fn try_floor_u64(&self) -> Result<u64, PaymentError> {
        let ceil_val = self
            .0
            .checked_div(Self::wad())
            .ok_or(PaymentError::MathOverflow)?;
...
impl TryAdd for Decimal {
    fn try_add(self, rhs: Self) -> Result<Self, PaymentError> {
        Ok(Self(
            self.0
                .checked_add(rhs.0)
                .ok_or(PaymentError::MathOverflow)?,
...
impl TrySub for Decimal {
    fn try_sub(self, rhs: Self) -> Result<Self, PaymentError> {
        Ok(Self(
            self.0
                .checked_sub(rhs.0)
                .ok_or(PaymentError::MathOverflow)?,
...
impl TryDiv<u64> for Decimal {
    fn try_div(self, rhs: u64) -> Result<Self, PaymentError> {
        Ok(Self(
            self.0
...
impl TryDiv<Decimal> for Decimal {
    fn try_div(self, rhs: Self) -> Result<Self, PaymentError> {
        Ok(Self(
            self.0
...
impl TryMul<u64> for Decimal {
    fn try_mul(self, rhs: u64) -> Result<Self, PaymentError> {
        Ok(Self(
            self.0
...
impl TryMul<Decimal> for Decimal {
    fn try_mul(self, rhs: Self) -> Result<Self, PaymentError> {
        Ok(Self(
            self.0
                .checked_mul(rhs.0)
                .ok_or(PaymentError::MathOverflow)?
